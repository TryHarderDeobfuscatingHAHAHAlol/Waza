local library = loadstring(game:HttpGet("https://raw.githubusercontent.com/IVRzHxZ/Kyo-UI-Library-llll/refs/heads/main/Kyo-Private", true))()
...
task.spawn(function()
    ...
    local FIRE_ORANGE = Color3.fromRGB(180, 120, 255)
    local DARK_EMBER = Color3.fromRGB(180, 120, 255)
...
local groundslamxstomp = mainTab:AddFolder("Ground Slam")
...
-- 1. Auto Ground Slam Switch
groundslamxstomp:AddSwitch("Auto Ground Slam", function(state)
    autoSlam = state
    ...
    task.spawn(function()
        while autoSlam do
            pcall(function()
                ...
                -- Equip Ground Slam / matching tools from backpack
                if character and backpack then
                    for _, tool in ipairs(backpack:GetChildren()) do
                        if tool:IsA("Tool") and string.find(string.lower(tool.Name), "ground slam") then
                            tool.Parent = character
                        end
                    end
                end
                
                -- Fire Ground Slam Event
                player.muscleEvent:FireServer(unpack({"groundSlam"}))
            end)
...
-- 2. Auto Stomp Switch
groundslamxstomp:AddSwitch("Auto Stomp", function(state)
    autoStomp = state
    ...
    task.spawn(function()
        while autoStomp do
            pcall(function()
                ...
                -- Equip Stomp / matching tools from backpack
                if character and backpack then
                    for _, tool in ipairs(backpack:GetChildren()) do
                        if tool:IsA("Tool") and string.find(string.lower(tool.Name), "stomp") then
                            tool.Parent = character
                        end
                    end
                end
                ...
                -- Fire Stomp Event
                player.muscleEvent:FireServer(unpack({"stomp"}))
            end)
...
-- Variables (Baguhin ang targetPos kung saan mo gustong mag-teleport)
local localPlayer = Players.LocalPlayer
local TeleportTime = 1 
local targetPos = Vector3.new(0, -1000, 0) -- Target position sa void
...
-- Function para gumawa o i-maintain ang Gaze part at Camera Subject
local function ensureGazePart(fallbackPos)
    local gaze = workspace:FindFirstChild("Gaze")
    if not gaze then
        gaze = Instance.new("Part")
        gaze.Size = Vector3.new(4, 1, 4)
        gaze.Position = fallbackPos or lastPos
        gaze.Anchored = true
        gaze.CanCollide = false
        gaze.Transparency = 0.5
        gaze.Name = "Gaze"
        gaze.Parent = workspace
    end
    
    if workspace.CurrentCamera and workspace.CurrentCamera.CameraSubject ~= gaze then
        workspace.CurrentCamera.CameraSubject = gaze
    end
    return gaze
end
...
local function startVelLoop()
    ...
    velConn = RunService.Heartbeat:Connect(function()
        if toggle then
            -- 🔥 [FIX] Gagamit ng NaN (0/0) para tuluyang ma-freeze/bypass ang void destruction limit
            workspace.FallenPartsDestroyHeight = 0/0
            
            -- Siguraduhing nakatutok ang camera sa Gaze part habang naka-ON
            ensureGazePart(lastPos)
            
            local char = localPlayer.Character
            local hrp = getHRP()
            
            -- [FIX] Auto-NoClip para hindi ka mag-rubberband sa lapag paitaas
            if char then
...
-- Toggle Feature sa Elerium v2
local switch = mainTab:AddSwitch("God Mode", function(bool)
    toggle = bool
    ...
    if toggle then
        ...
        task.wait()
        workspace.FallenPartsDestroyHeight = 0/0 -- 🔥 Void bypass instant activation
        local tweenToTarget = createTween(CFrame.new(targetPos))
        if tweenToTarget then
            tweenToTarget:Play()
        ...
        local destPos = currentHrp and currentHrp.Position or lastPos
        local tweenBack = createTween(CFrame.new(destPos))
        if tweenBack then
            tweenBack:Play()
            tweenBack.Completed:Wait()
        end
        workspace.FallenPartsDestroyHeight = -500 -- Ibalik sa normal kapag naka-OFF
        resetCameraSubject()
...
-- 🔄 Keep On hanggang pag-respawn (Naka-lock pa rin ang camera sa Gaze part)
...
local function hideElement(element)
    ...
    element.Position = UDim2.new(5, 0, 5, 0) -- Teleport far off-screen
end
...
-- Create Tab
local upgradesTab = window:AddTab("Ultimates")

upgradesTab:AddLabel("Noted: You need to respawn after upgrading.")

-- Helper function to create a looping switch
local function addLoopSwitch(tab, upgradeName, delayTime)
    delayTime = delayTime or 0.1 -- Adjust loop speed if needed
    local active = false

    tab:AddSwitch(upgradeName, function(state)
        active = state
        if active then
            task.spawn(function()
                local args = {
...
-- Add Looping Switches for each upgrade
addLoopSwitch(upgradesTab, "+5% Rep Speed")
addLoopSwitch(upgradesTab, "+1 Pet Slot")
addLoopSwitch(upgradesTab, "+10 Item Capacity")
addLoopSwitch(upgradesTab, "+1 Daily Spin")
addLoopSwitch(upgradesTab, "+10% Lucky")
addLoopSwitch(upgradesTab, "x2 Quest Rewards")
addLoopSwitch(upgradesTab, "Muscle Mind")
addLoopSwitch(upgradesTab, "Jungle Swift")
addLoopSwitch(upgradesTab, "Infernal Health")
addLoopSwitch(upgradesTab, "Galaxy Gains")
addLoopSwitch(upgradesTab, "Demon Damage")
addLoopSwitch(upgradesTab, "Golden Rebirth")

local mainTab = window:AddTab("Crystals")

-- State variables
local autoOpenActive = false
local selectedCrystal = "Chaos Crystal" -- Default tracking option
...
local autosellv1 = mainTab:AddFolder("Auto Sell")

-- The specific sellPet remote script wrapped inside an AddSwitch loop
autosellv1:AddSwitch("Auto Sell Alien Girl", function(state)
...
-- The specific sellPet remote script wrapped inside an AddSwitch loop
autosellv1:AddSwitch("Auto Sell Ice Ice", function(state)
    getgenv().AutoSell = state
...
-- The specific sellPet remote script wrapped inside an AddSwitch loop
autosellv1:AddSwitch("Auto Sell Slitherman", function(state)
    getgenv().AutoSell = state
...
-- The specific sellPet remote script wrapped inside an AddSwitch loop
autosellv1:AddSwitch("Auto Sell Smiley", function(state)
    getgenv().AutoSell = state
...
-- The specific sellPet remote script wrapped inside an AddSwitch loop
autosellv1:AddSwitch("Auto Sell Mask Of Gold", function(state)
    getgenv().AutoSell = state
...
-- The specific sellPet remote script wrapped inside an AddSwitch loop
autosellv1:AddSwitch("Auto Sell Sir Flame", function(state)
    getgenv().AutoSell = state
...
-- The specific sellPet remote script wrapped inside an AddSwitch loop
autosellv1:AddSwitch("Auto Sell Dark Soul Birdie", function(state)
    getgenv().AutoSell = state
...
-- The specific sellPet remote script wrapped inside an AddSwitch loop
autosellv1:AddSwitch("Shadow Eclipse Leviathan", function(state)
    getgenv().AutoSell = state
...
-- The specific sellPet remote script wrapped inside an AddSwitch loop
autosellv1:AddSwitch("Auto Sell Shadows Edge Kitty", function(state)
    getgenv().AutoSell = state
...
local autoevolvedv1 = mainTab:AddFolder("Auto Evolved")
...
autoevolvedv1:AddSwitch("Auto Evolve All Pets", function(state)
...
-- Add a functional switch into the Automation tab
autoevolvedv1:AddSwitch("Auto Evolve Hank", function(state)
    autoEvolveEnabled = state
                    ...
                    for _, category in ipairs(petsFolder:GetChildren()) do
                        ...
                        if category:IsA("Folder") or category:IsA("Configuration") then
                            for _, pet in ipairs(category:GetChildren()) do
                                if pet.Name:lower():find("hank") then
                                    foundPet = pet
                                    break
                                end
                            end
...
-- Add a functional switch into the Automation tab
autoevolvedv1:AddSwitch("Auto Evolve Gold Warrior", function(state)
    autoEvolveEnabled = state
            ...
            while autoEvolveEnabled do
                ...
                if petsFolder then
                    for _, category in ipairs(petsFolder:GetChildren()) do
                        ...
                        if category:IsA("Folder") or category:IsA("Configuration") then
                            for _, pet in ipairs(category:GetChildren()) do
                                if pet.Name:lower():find("gold warrior") then
                                    foundPet = pet
                                    break
                                end
                            end
...
-- Add a functional switch into the Automation tab
autoevolvedv1:AddSwitch("Auto Evolve Sky Hawk", function(state)
    autoEvolveEnabled = state
            ...
            while autoEvolveEnabled do
                ...
                if petsFolder then
                    for _, category in ipairs(petsFolder:GetChildren()) do
                        ...
                        if category:IsA("Folder") or category:IsA("Configuration") then
                            for _, pet in ipairs(category:GetChildren()) do
                                if pet.Name:lower():find("sky hawk") then
                                    foundPet = pet
                                    break
                                end
                            end
...
autoevolvedv1:AddSwitch("Auto Evolve Killer Pheonix", function(state)
    autoEvolveEnabled = state
            ...
            while autoEvolveEnabled do
                ...
                if petsFolder then
                    for _, category in ipairs(petsFolder:GetChildren()) do
                        ...
                        if category:IsA("Folder") or category:IsA("Configuration") then
                            for _, pet in ipairs(category:GetChildren()) do
                                if pet.Name:lower():find("killer pheonix") then
                                    foundPet = pet
                                    break
                                end
                            end
...
-- Add a functional switch into the Automation tab
autoevolvedv1:AddSwitch("Auto Evolve Dual Chaos Birdies", function(state)
    autoEvolveEnabled = state
            ...
            while autoEvolveEnabled do
                ...
                if petsFolder then
                    for _, category in ipairs(petsFolder:GetChildren()) do
                        ...
                        if category:IsA("Folder") or category:IsA("Configuration") then
                            for _, pet in ipairs(category:GetChildren()) do
                                if pet.Name:lower():find("dual chaos birdies") then
                                    foundPet = pet
                                    break
                                end
                            end
...
-- Add a functional switch into the Automation tab
autoevolvedv1:AddSwitch("SENSEI: Legend Of Weakness", function(state)
    autoEvolveEnabled = state
...
-- Add Switch (Toggle) to Elerium V2 Tab
mainTab:AddSwitch("Auto Equip All Pets", function(state)
    ...
    if autoEquipEnabled then
        task.spawn(function()
            while autoEquipEnabled do
                local player = game:GetService("Players").LocalPlayer
                local replicatedStorage = game:GetService("ReplicatedStorage")
                local equipEvent = replicatedStorage:WaitForChild("rEvents"):WaitForChild("equipPetEvent")
                    ...
                    for _, category in ipairs(petsFolder:GetChildren()) do
                        ...
                        if category:IsA("Folder") or category:IsA("Configuration") then
                            for _, pet in ipairs(category:GetChildren()) do
                                ...
                                pcall(function()
                                    equipEvent:FireServer("equipPet", pet.Name)
                                end)
                                pcall(function()
                ...
                -- Wait a bit before repeating the loop if switch is still on
                task.wait(5)
            end
        end)
    end
...
-- Add Button to Elerium V2 Tab
mainTab:AddButton("Equip All Pets", function()
    local player = game:GetService("Players").LocalPlayer
    local replicatedStorage = game:GetService("ReplicatedStorage")
    local equipEvent = replicatedStorage:WaitForChild("rEvents"):WaitForChild("equipPetEvent")
    ...
    if petsFolder then
        for _, category in ipairs(petsFolder:GetChildren()) do
            if category:IsA("Folder") or category:IsA("Configuration") then
                for _, pet in ipairs(category:GetChildren()) do
                    -- Fire multiple argument structures simultaneously to catch whatever the server validates
                    pcall(function()
                        equipEvent:FireServer("equipPet", pet.Name)
                    end)
                    pcall(function()
...
-- Add Button to Unequip All Pets
mainTab:AddButton("Unequip All Pets", function()
    local player = game:GetService("Players").LocalPlayer
    local replicatedStorage = game:GetService("ReplicatedStorage")
    local equipEvent = replicatedStorage:WaitForChild("rEvents"):WaitForChild("equipPetEvent")

    -- Method 1: Try common bulk unequip strings used in simulators
    pcall(function()
        equipEvent:FireServer("unequipAllPets")
    end)
    ...
    -- Method 2: If the game requires looping through equipped/stored pets to unequip them individually
    local petsFolder = player:FindFirstChild("petsFolder")
    if petsFolder then
        for _, pet in ipairs(petsFolder:GetDescendants()) do
            if not pet:IsA("Folder") and not pet:IsA("Configuration") and pet.Name ~= "petsFolder" then
                pcall(function()
                    -- Trying common variations for individual pet removal via the event
                    equipEvent:FireServer("unequipPet", pet.Name)
                    equipEvent:FireServer("unequipPet", pet)
                    equipEvent:FireServer("unequip", pet.Name)
                end)
                task.wait(0.02)
            end
        end
    ...
    print("Attempted to unequip all pets!")
end)
